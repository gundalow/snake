...
class Food:
    def __init__(self, x, y, food_type=FoodType.NORMAL, food_name=""):
        ...
        self.bite_cooldown = 0 # Added bite cooldown to prevent instant consumption
    ...
    def update(self, current_time, delta_time):
        # Elastic growth animation logic
        dt = current_time - self.spawn_time
        if dt < 0.75:
            # Simple elastic ease out approximation
            p = dt / 0.75
    ...
    def take_bite(self):
        if self.bite_cooldown > 0:
            return False # Cannot take another bite yet

        self.bites_remaining -= 1
        self.bite_cooldown = 0.5 # 0.5s between bites
    ...
    def draw(self, screen):
        ...
        if self.food_type == FoodType.MEGA:
            # Visually indicate bites remaining
...
class FoodSpawner:
    ...
    def spawn_food(self, snake):
        ...
        while not valid_pos and attempts < 50:
            ...
            # Check snake head
            if pygame.Vector2(x, y).distance_to(snake.pos) < grid_size * 2:
                valid_pos = False
                continue
